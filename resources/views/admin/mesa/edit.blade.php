...
@section('content')
    <div class="card" style="width: 30%">
        <div class="card-body">
            <form action="{{ url('/mesa/'.$mesa->id) }}" method="post">
            @csrf
            {{ @method_field('PATCH') }}
            <div class="form-group">
                <label for="nombre" >Nombre Mesa: </label>
                <input type="text" name="Nombre_mesa" id="Nombre_mesa" value="{{ old('Nombre_mesa', $mesa->Nombre_mesa) }}"
            ...
            <center>
                ...
                <a href="{{ route('admin.mesa.index') }}" class="btn btn-secondary" tabindex="4">Cancelar</a>
            </center>
            </form>
        </div>
    </div>
